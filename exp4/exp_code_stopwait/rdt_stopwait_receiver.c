...
int receive_file( char *save_file_name, int sock_fd )
{
	...
	pkt_len = recvfrom(sock_fd, rdt_pkt, RDT_PKT_LEN, 0,
			(struct sockaddr_in*)&client_addr, &sin_len);
	...
	if(flag == RDT_CTRL_END){
        ...
        break;
			...
			reply_pkt_len = pack_rdt_pkt(NULL, reply_pkt_buf, 0,
			seq_num, RDT_CTRL_ACK);
			udt_sendto(sock_fd, reply_pkt_buf, reply_pkt_len, 0, 
